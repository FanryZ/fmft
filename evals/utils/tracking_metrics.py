...
def compute_tapvid_metrics_for_video(
    ...
    """
    # print(f"Looking for video_idx={video_idx} in benchmark_data")
    # print("Available video_idx values:", [v["video_idx"] for v in benchmark_data["videos"]])
    ...
    pred_occluded_list = []
    pred_tracks_list = []
    # print("Available frames in trajectories_dict:", sorted(trajectories_dict.keys()))
    # print("Frames requested by benchmark:", sorted(benchmark_video_data['query_points'].keys()))
...
def compute_badja_metrics_for_video(
    ...
    for frame_idx in benchmark_video_data['target_points']:
        trajectories = trajectories_dict[frame_idx] # N' x T x 3
